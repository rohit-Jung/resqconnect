...
function PaymentSuccessContent() {
  ...
  const verifyMutation = useVerifyPaymentByPidx();
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetPaymentById(paymentId || '', !!paymentId && !pidx);

  const {
    data: paymentByPidx,
    isLoading: isLoadingPidx,
    isError: isErrorPidx,
  } = useGetPaymentByPidx(pidx || '', !!pidx);

  // Khalti redirect to `return_url` does not update our DB.
  // In local dev (no webhooks), we verify here by calling control-plane.
  useEffect(() => {
    if (!pidx) return;
    if (verifyMutation.isPending) return;
    // Avoid spamming: only verify once per page load.
    if (didVerifyRef.current) return;
    didVerifyRef.current = true;
    verifyMutation.mutate({ pidx });
  }, [pidx, verifyMutation, didVerifyRef]);

  const payment = (pidx ? paymentByPidx : paymentData)?.data?.payment;
  const anyLoading = isLoading || isLoadingPidx;
  const anyError = isError || isErrorPidx;
