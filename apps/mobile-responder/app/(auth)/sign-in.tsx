...
import { useLoginServiceProvider } from '@/services/user/auth.api';
...
const SigninScreen: React.FC = () => {
  ...
  const { mutate: loginServiceProviderMutation, isPending } =
  ...
  async function loginEntity(
    ...
    if (isLoginResponse(responseData)) {
      ...
      const userId = responseData.serviceProviderId;
  ...
  const onSubmit = async (data: TLoginUser) => {
    if (!siloBaseUrl) {
      Alert.alert(
        'Select Organization',
        'Please select your organization first.'
      );
      return;
    }
      ...
      <Pressable style={styles.container} onPress={Keyboard.dismiss}>
        ...
        >
          ...
          >
            ...
            {/* Org routing selection (multi-silo) */}
            <View style={styles.formContainer}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <Text style={[styles.subtitle, { flex: 1 }]}>
                  Org: {orgName || 'Selected'}
                </Text>
                <TouchableOpacity
                  onPress={async () => {
                    await clearOrgRouting();
                    router.replace('/(auth)/select-organization');
                  }}
                  activeOpacity={0.7}
                >
            ...
            <View style={styles.formContainer}>
              ...
              {/* Responder accounts are provisioned by an organization/admin. */}
            </View>
