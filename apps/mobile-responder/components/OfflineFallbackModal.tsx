...
export const OfflineFallbackModal: React.FC<OfflineFallbackModalProps> = ({
  ...
  useEffect(() => {
    if (visible && location) {
      addToQueue({
        ...
        userPhone: provider?.phoneNumber?.toString(),
      });
    }
  ...
  }, [location, emergencyType, description, provider, onClose, router]);
  ...
  const handleGoToSMSScreen = useCallback(() => {
    ...
    Alert.alert(
      'SMS Fallback',
      'Please use your phone messaging app to send an emergency SMS.'
    );
