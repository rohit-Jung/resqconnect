...
import { notifyServiceProvider, notifyUser } from '@/services/notification.service';
...
const registerServiceProvider = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!parsedValues.success) {
    ...
    const validationError = new ApiError(
      ...
      parsedValues.error.issues.map(issue => `${String(issue.path[0])} : ${issue.message} `)
    );
  ...
  if (existingOrganization.serviceCategory !== parsedValues.data.serviceType) {
    throw new ApiError(400, 'Service Type does not match with organization service category');
  }
...
const loginServiceProvider = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!existingServiceProvider) {
    throw new ApiError(404, 'ServiceProvider not found with given credentials');
  }
...
const logoutServiceProvider = asyncHandler(async (req: Request, res: Response) => {
  ...
  res
    .status(200)
    .clearCookie('token')
    .json(new ApiResponse(200, 'Service Provider logged out successfully', {}));
});

const verifyServiceProvider = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!existingServiceProvider.verificationToken || !existingServiceProvider.tokenExpiry) {
...
const updateServiceProvider = asyncHandler(async (req: Request, res: Response) => {
  const loggedInServiceProvider = req.user;
...
const deleteServiceProvider = asyncHandler(async (req: Request, res: Response) => {});

const forgotServiceProviderPassword = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!existingServiceProvider) {
    throw new ApiError(404, 'ServiceProvider not found with given phone number');
  }
...
const resetServiceProviderPassword = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!existingServiceProvider.resetPasswordTokenExpiry) {
    throw new ApiError(400, 'Reset password token expiry not registered. Please verify again.');
  }
...
const getServiceProviderProfile = asyncHandler(async (req: Request, res: Response) => {
  const loggedInServiceProvider = req.user;
...
const updateServiceProviderStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (io && updatedProvider[0]) {
    io.to(`provider:${updatedProvider[0].id}`).emit(socketEvents.NOTIFICATION_RECEIVED, {
      type: 'status_updated',
  ...
  if (emergencyResponseId && (status === 'off_duty' || status === 'unavailable')) {
    // Get the emergency response details
    const emergencyResponseDetails = await db.query.emergencyResponse.findFirst({
      where: eq(emergencyResponse.id, emergencyResponseId),
    });

    if (emergencyResponseDetails && emergencyResponseDetails.emergencyRequestId) {
      // Get the emergency request details
      const emergencyRequestDetails = await db.query.emergencyRequest.findFirst({
        where: eq(emergencyRequest.id, emergencyResponseDetails.emergencyRequestId),
      });
...
const getNearbyProviders = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!latitude || !longitude || !serviceType) {
    throw new ApiError(400, 'latitude, longitude, and serviceType are required');
  }
  ...
  const withDistance = providers
    .filter(p => p.currentLocation && p.currentLocation.latitude && p.currentLocation.longitude)
    .map(p => ({
      ...p,
...
const changeProviderPassword = asyncHandler(async (req: Request, res: Response) => {
  ...
  const isPasswordValid = await bcrypt.compare(oldPassword, existingProvider.password);
...
const updateServiceProviderLocation = asyncHandler(async (req: Request, res: Response) => {
  const loggedInProvider = req.user;
  ...
  if (!currentLocation || !currentLocation.latitude || !currentLocation.longitude) {
    throw new ApiError(400, 'Invalid location data. Provide latitude and longitude');
  }
